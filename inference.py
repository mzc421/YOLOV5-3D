...
def detect3d(weights, source, data, imgsz, device, classes, reg_weights, model_select, calib_file, output_path,
             show_result=None, save_result=None):
    # Directory
    ...
    for i, img_path in enumerate(imgs_path):
        ...
        # Run detection 2d YOLO目标检测
        dets = detect2d(model, img_path, stride, pt, imgsz, device, classes, names)
...
@torch.no_grad()
def detect2d(model, source, stride, pt, imgsz, device, classes, names):
...
def main(opt):
    detect3d(weights=opt.weights, source=opt.source, data=opt.data, imgsz=opt.imgsz, device=opt.device,
             classes=opt.classes, reg_weights=opt.reg_weights, model_select=opt.model_select, calib_file=opt.calib_file,
             output_path=opt.output_path, show_result=opt.show_result, save_result=opt.save_result)
