...
def upload_file():
    ...
    if 'image' in request.files:
        ...
        # process file
        detect3d(weights="weights/yolov5s.pt", source='static', data="data/coco128.yaml", imgsz=[640, 640], device="0",
                 classes=opt.classes, reg_weights=opt.reg_weights, model_select=opt.model_select,
                 calib_file=opt.calib_file, output_path=opt.output_path, save_result=opt.save_result)
...
def parse_opt():
    ...
    parser.add_argument('--reg_weights', type=str, default='weights/resnet.pkl', help='Regressor model weights')
    ...
    parser.add_argument('--output_path', type=str, default=ROOT / 'static', help='Save output pat')
